...
// Subcomponent for Blurred Text (Reusable)
const BlurredText = ({ 
    text, 
    className = "", 
    blurAmount = "blur-[6px]", 
    hoverBlur = "hover:blur-[3px]",
  ...
  return (
    <p 
      onClick={handleClick}
      className={`transition-all duration-700 cursor-pointer select-none ${isBlurred ? `${blurAmount} ${hoverBlur}` : 'blur-0'} ${className}`}
...
export const TranscriptView: React.FC<TranscriptViewProps> = ({ segments, currentTime, onSegmentClick, meta, onToggleFavorite }) => {
  ...
  const handlePlayOriginalSegment = (e: React.MouseEvent, startTime: number) => {
      e.stopPropagation();
            ...
            >
              ...
              {onToggleFavorite && (
                  <button 
                    ...
                    className={`absolute top-4 right-4 p-1 rounded-full transition-colors z-10 ${segment.isFavorite ? 'text-yellow-400 hover:text-yellow-500' : 'text-slate-200 hover:text-slate-400'}`}
                  >
              ...
              {/* Original Text Row with Play Button */}
              <div className="flex items-start gap-3 mb-4 w-full pr-8">
                 {/* Play Button for Original Sentence */}
                 <button 
                    onClick={(e) => handlePlayOriginalSegment(e, segment.start)}
                    className="mt-1 p-1.5 rounded-full bg-slate-100 text-slate-500 hover:bg-blue-100 hover:text-blue-600 transition-colors shrink-0"
                 ...
                 <div className="flex-1 min-w-0">
                    <p className={`text-base break-words whitespace-pre-wrap leading-relaxed ${isActive ? 'text-slate-900 font-medium' : 'text-slate-600'}`}>
                  ...
                  <div className="mb-4 bg-blue-50 rounded-lg p-3 border-l-4 border-blue-400 shadow-sm overflow-hidden">
                    <div className="mb-3">
                        <div className="text-[10px] uppercase font-bold text-blue-400 tracking-wider mb-1 flex items-center gap-1">
                            <span>🇺🇸 American Native Rewrite</span>
                        </div>
                        <p className="text-base text-blue-900 font-medium leading-relaxed break-words whitespace-pre-wrap">
                            "{segment.idiomatic}"
                        </p>
                    </div>
                    
                    {/* Explanation (Why is this better?) */}
                    {segment.idiomExplanation && (
                        <div className="mb-3 pl-2 border-l-2 border-blue-200">
                             <BlurredText 
                                text={segment.idiomExplanation}
                                className="text-xs text-blue-800/80 leading-snug"
                                blurAmount="blur-[4px]"
                                autoHideDuration={5000} // 5 Seconds reveal
                             />
                        </div>
                    )}
              ...
              {/* Translation - Moved to bottom and smaller font */}
